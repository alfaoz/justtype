...
export function RecoveryKeyModal({ recoveryPhrase, onAcknowledge, subtitle }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/30 backdrop-blur-md animate-modal-overlay z-[60] flex items-center justify-center p-4">
      <div className="bg-[var(--theme-bg-secondary)] border border-[var(--theme-border)] rounded animate-modal-content p-6 md:p-8 max-w-lg w-full" onClick={(e) => e.stopPropagation()}>
        ...
        <p className="text-[var(--theme-text-muted)] text-sm mb-2">{strings.auth.recoveryKey.description}</p>
        <p className="text-red-400 text-sm mb-4">{strings.auth.recoveryKey.warning}</p>

        <div className="bg-[var(--theme-bg)] border border-[var(--theme-border)] rounded p-4 mb-4 font-mono text-sm text-white leading-relaxed select-all break-words">
          {recoveryPhrase}
        </div>
        ...
        <div className="flex gap-3 mb-6">
          <button
            ...
            className="flex-1 border border-[var(--theme-border)] text-white px-4 py-2.5 rounded hover:bg-[var(--theme-bg-tertiary)] transition-colors text-sm"
          >
        ...
        <label className="flex items-start gap-3 mb-4 cursor-pointer select-none">
          ...
          />
          <span className="text-[var(--theme-text-muted)] text-sm">{strings.auth.recoveryKey.acknowledge}</span>
        </label>
