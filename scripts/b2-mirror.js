...
(async () => {
  ...
  fs.writeFileSync(path.join(dir, 'manifest.json'), JSON.stringify({ takenAt: new Date().toISOString(), prefix, count: files.length, bytes, files: manifest }, null, 2));
  console.log(`${dir}: ${files.length} files, ${bytes} bytes`);
